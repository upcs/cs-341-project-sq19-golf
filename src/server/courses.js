...
module.exports = {
  parseCourseData: (filePath) => {
    ...
    let courses = lines.filter(line => line != '')
      ...
      return new Course(fields[0].toUpperCase().trim(), fields[1].trim(), fields[2].trim(), fields[3].trim(), 
						fields[4].toLowerCase().trim(), fields[5].toLowerCase().trim(), fields[6].toUpperCase().trim(), 
						fields[7].trim(), fields[8].trim(), fields[9].toLowerCase().trim(), fields[10].toLowerCase().trim());
    });
